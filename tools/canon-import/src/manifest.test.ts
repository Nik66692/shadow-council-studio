...
import { canonicalManifestRelativePath, createManifest, sha256File } from './manifest';
...
describe('canon import manifest', () => {
  it('creates a deterministic manifest for a valid source', () => {
    const root = makeRoot();
    const source = 'docs/canon/source/v1.3/Shadow_Council_Source_of_Truth_v1.3.docx';
  ...
  it('fails when the manifest source is missing', () => {
    ...
    expect(() => createManifest(root)).toThrow(/source referenced by manifest is missing/);
  });
