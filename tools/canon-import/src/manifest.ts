...
export function readSourceManifest(rootDir: string): SourceManifest {
  ...
  if (!existsSync(manifestPath)) {
    throw new CanonManifestError(
      `Canonical source manifest is missing: ${canonicalManifestRelativePath}`,
    );
  }
  ...
  if (
    ...
    throw new CanonManifestError(
      "Canonical source manifest is invalid or incomplete.",
    );
  }
...
export function createManifest(
  ...
  if (!existsSync(sourcePath)) {
    throw new CanonManifestError(
      `Canonical source referenced by manifest is missing: ${sourceManifest.currentSource}`,
    );
  }
  ...
  return {
    ...
    generatedAt,
    warnings: [
      "semantic DOCX extraction is intentionally not implemented in Sprint 0",
    ],
    semanticExtraction: "not-implemented-in-sprint-0",
  };
}
