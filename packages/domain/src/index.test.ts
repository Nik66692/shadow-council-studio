...
describe('domain foundation', () => {
  ...
  it('validates health contracts', () => {
    expect(HealthStatusSchema.parse({ projectName:'Shadow Council Studio', developmentStage:'Foundation', databaseConnected:true, migrationsApplied:true, sourceOfTruth:{ exists:false, filename:'docs/canon/source/manifest.json', sha256:null, canonVersion:null }, modulesImplemented:['Dashboard'], nextRecommendedPhase:'Phase 1', diagnostics:[]}).databaseConnected).toBe(true);
  });
});
